#!/usr/bin/env python3
"""
Ultra-efficient LTDv2 COCO to YOLO converter.
Processes annotations in chunks to avoid memory issues.
"""
import json
import logging
...
def load_json_iteratively(json_path: Path):
...
def convert_split(json_path: Path, frames_dir: Path, output_dir: Path, split: str):
    ...
    # Load JSON
    data = load_json_iteratively(json_path)
    ...
    # PASS 1: Group annotations by image ID (process in chunks)
    logger.info("Grouping annotations by image...")
    ...
    if 'annotations' in data:
        ...
        for i in range(0, total_ann, chunk_size):
            ...
            if (i + chunk_size) % (chunk_size * 5) == 0:
                logger.info(f"  Processed {min(i+chunk_size, total_ann):,}/{total_ann:,} annotations")
...
def main():
    logger.info("="*60)
    logger.info("LTDv2 COCO → YOLO Converter")
    logger.info("="*60)
    ...
    test_data = load_json_iteratively(test_json)
    logger.info(f"Images: {len(test_data['images']):,}")
